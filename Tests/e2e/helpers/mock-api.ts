...
export interface MockCouponLike {
  code: string;
...
export interface MockApiOptions {
  ...
  /** Active admin coupons shared with storefront validate (optional). */
  getActiveCoupons?: () => MockCouponLike[];
...
export async function installStorefrontApiMocks(
  ...
  const getActiveCoupons = options.getActiveCoupons;
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (path === '/coupons/validate' && method === 'POST') {
      ...
      const sharedCoupon = getActiveCoupons?.().find((c) => c.code.toUpperCase() === code && c.is_active);
      const legacyCoupon =
        code === 'LDCOFF10'
          ? { code: 'LDCOFF10', discount_type: 'percentage' as const, discount_value: 10, is_active: true }
          : null;
      const matched = sharedCoupon ?? legacyCoupon;
      ...
      if (matched) {
        ...
        return json(route, {
          ...
          valid: true,
          coupon: {
            id: `mock-coupon-${matched.code.toLowerCase()}`,
            code: matched.code,
            description: `${matched.discount_value}${matched.discount_type === 'percentage' ? '%' : '$'} off`,
          ...
          message: `${matched.discount_value}${matched.discount_type === 'percentage' ? '%' : ''} discount applied!`,
        });
      }
      return json(route, {
        success: true,
