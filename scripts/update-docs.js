...
const DOC = path.join(__dirname, '..', 'documentation');
const INFO = '../info.md';

function write(name, content) {
  fs.writeFileSync(path.join(DOC, name), content.trim() + '\n');
  console.log('  ' + name);
}
...
for (const [name, content] of Object.entries(SPECIAL)) {
  write(name, content);
}
for (const [name, [title, topic]] of Object.entries(GENERIC)) {
  write(name, ref(title, `This document describes **${topic}**.\n\nSee [\`${INFO}\`](${INFO}) for the complete reference including architecture, security, payments, logging, and API documentation.`));
}

const MEDIA = path.join(__dirname, '..', 'frontend', 'src', 'assets', 'MEDIA_ASSET_GUIDE.md');
const mediaContent = `# Media Asset Guide
...
`;
fs.writeFileSync(MEDIA, mediaContent.trim() + '\n');
console.log('  frontend/src/assets/MEDIA_ASSET_GUIDE.md');
console.log('Done.');
