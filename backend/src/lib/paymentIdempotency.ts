...
export async function ensureIdempotencyTable(): Promise<void> {
  await sql.begin(async (tx) => {
    ...
    `;
    await tx`CREATE INDEX IF NOT EXISTS idx_payment_idempotency_expires ON payment_idempotency (expires_at)`;
    await tx`CREATE INDEX IF NOT EXISTS idx_payment_idempotency_operation ON payment_idempotency (operation)`;
  });
}
