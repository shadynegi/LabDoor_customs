...
var LiquidWeb = (() => {
  ...
  // src/liquid-core.mjs
  var liquid_core_exports = {};
  __export(liquid_core_exports, {
    default: () => liquid_core_default
  });

  // src/core/errors/LiquidWebError.mjs
  var LiquidWebError = class extends Error {
  ...
  // src/core/methods/events-emitter.mjs
  ...
  var destroy_default = {
    destroy() {
      ...
      this.emit("destroy");
      if (this.$el) {
        core_default.__instances__.delete(this.$el);
  ...
  // src/core/methods/public-api.mjs
  var publicMethods = {
    getInstance(el) {
      const instance = new this(void 0, { init: false });
  ...
  var LiquidWebError2 = class extends Error {
    constructor(message, originalError) {
      super(message);
      this.name = "LiquidWebError";
      this.originalError = originalError;
  ...
  var DOMManager = class {
    ...
    validateElement(el) {
      if (!el) {
        throw new LiquidWebError2("Element is required");
      }
      if (!(el instanceof Element)) {
        throw new LiquidWebError2("Invalid element type. Expected HTMLElement");
      }
    }
    createStructure(el) {
      this.$el = el;
      if (this.$el.children.length > 1) {
        throw new LiquidWebError2(
  ...
  function isNode(node) {
    return typeof window !== "undefined" && typeof window.HTMLElement !== "undefined" ? node instanceof window.HTMLElement : node && typeof node === "object" && node.nodeType === 1 && typeof node.nodeName === "string";
  }

  // src/utils/deepMerge.mjs
  ...
  var LiquidWeb = class _LiquidWeb {
    static __instances__ = /* @__PURE__ */ new Map();
    ...
    constructor(...args) {
      ...
      if (this.$el && typeof this.$el === "string") {
        ...
        const liquidwebArray = Array.from(elements).map((el2) => {
          if (_LiquidWeb.__instances__.has(el2)) {
            return _LiquidWeb.__instances__.get(el2);
          }
          const newOptions = deepMerge({}, options, { el: el2 });
          const instance = new _LiquidWeb(newOptions);
          _LiquidWeb.__instances__.set(el2, instance);
          return instance;
        });
        return new Proxy(liquidwebArray, {
          get(target, prop) {
            ...
            if (multiInstanceMethods.includes(prop)) {
              return (...args2) => {
                target.map((instance) => {
                  return instance[prop].apply(instance, args2);
                });
                return target;
              };
            }
      ...
      this.options = deepMerge({}, options_default, options);
      ...
      if (!_LiquidWeb.__stylesInjected__) {
        _LiquidWeb.injectStyles();
      }
      if (this.$el instanceof Element) {
        _LiquidWeb.__instances__.set(this.$el, this);
    ...
    static injectStyles() {
      if (_LiquidWeb.__stylesInjected__ || typeof document === "undefined") {
        return;
      }
      const css = `
      .liquidweb {
    ...
    `;
      _LiquidWeb.__styleElement__ = document.createElement("style");
      _LiquidWeb.__styleElement__.type = "text/css";
      _LiquidWeb.__styleElement__.appendChild(document.createTextNode(css));
      document.head.appendChild(_LiquidWeb.__styleElement__);
      _LiquidWeb.__stylesInjected__ = true;
    }
    createSVGFilter() {
    ...
    addChannelDisplacement(filter, channel) {
      const scale = this.options.scale * (this.options.mode === "shader" ? 1 : -1);
    ...
    updateEffects() {
      ...
      if (this.$backdrop) {
        const blur = typeof this.options.blur === "number" ? `${this.options.blur}px` : this.options.blur;
        const saturation = typeof this.options.saturation === "number" ? `${this.options.saturation}%` : this.options.saturation;
  ...
  var core_default = LiquidWeb;

  // src/liquid-core.mjs
  var liquid_core_default = core_default;
  return __toCommonJS(liquid_core_exports);
})();
window.LiquidWeb = LiquidWeb.default || LiquidWeb; })();

//# sourceMappingURL=liquid-core.js.map
