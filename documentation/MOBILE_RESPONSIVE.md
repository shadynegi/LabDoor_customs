...
**Automated baseline:** Playwright **`responsive-pages-ui.spec.ts`** (mobile-chrome project) runs **168 tests** across **10 phone viewports** (iPhone SE, iPhone 15 Pro, iPhone 17 / 17 Pro / 17 Pro Max, Galaxy S24 / S25 / S25 Ultra / A55, Pixel 9 Pro) and **every storefront route** (home, products, product detail, cart, checkout, about, contact, help, policies, orders, payment success/cancel, admin login). Each viewport checks horizontal overflow, primary content visibility, cart **All sales final** above the sticky checkout bar, and checkout form fields. **`storefront.spec.ts`** adds shipping-policy pricing, contact email, and document-scroll smoke. **`responsive-ui.spec.ts`** adds focused mobile checkout/cart/sort/admin cases. Full suite: **411 tests** — see [`test_guidelines.md`](test_guidelines.md).
