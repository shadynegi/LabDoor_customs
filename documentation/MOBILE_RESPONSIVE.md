...
**Automated baseline:** Playwright **`responsive-pages-ui.spec.ts`** (mobile-chrome project) runs **168 tests** across **10 phone viewports** (iPhone SE, iPhone 15 Pro, iPhone 17 / 17 Pro / 17 Pro Max, Galaxy S24 / S25 / S25 Ultra / A55, Pixel 9 Pro) and **every storefront route** (home, products, product detail, cart, checkout, about, contact, help, policies, orders, payment success/cancel, admin login). Each viewport checks horizontal overflow, primary content visibility, cart **All sales final** above the sticky checkout bar, and checkout form fields. **`responsive-ui.spec.ts`** adds focused mobile checkout/cart/sort/admin cases. See [`test_guidelines.md`](test_guidelines.md).
...
- **Checkout keyboard:** `keyboardOffset` lifts the sticky **Place Order** CTA above the virtual keyboard (`translateY`). Extra bottom padding when policy/validation hints show (`has-sticky-hint`).
...
- **Body scroll:** `html` and `body` use `min-height` (not fixed `height: 100%`), `overflow-y: auto`, `overscroll-behavior-y: contain`, and `-webkit-fill-available` so tall pages (cart, checkout) scroll correctly on phones; horizontal overflow is clipped.
