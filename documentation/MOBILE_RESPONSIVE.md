...
**Automated baseline:** Playwright **`Tests/e2e/specs/responsive/responsive-pages-ui.spec.ts`** (mobile-chrome project) runs **193 tests** across **11 phone viewports** (iPhone SE 320px, iPhone SE 3rd gen, iPhone 15 Pro, iPhone 17 / 17 Pro / 17 Pro Max, Galaxy S24 / S25 / S25 Ultra / A55, Pixel 9 Pro) and **every storefront route** (home, products, product detail, cart, checkout, about, contact, help, policies, orders, payment success/cancel, admin login). Each viewport checks horizontal overflow, primary content visibility, cart **All sales final** above the sticky checkout bar, and checkout form fields. **`Tests/e2e/specs/storefront/storefront.spec.ts`** adds shipping-policy pricing, contact WhatsApp link, and document-scroll smoke. **`Tests/e2e/specs/contact/contact-ui.spec.ts`** asserts submit opens a `wa.me` popup with prefilled message. **`Tests/e2e/specs/responsive/responsive-ui.spec.ts`** adds focused mobile checkout/cart/sort/admin cases. Full suite (**520** tests + viewport audit): see [`test_guidelines.md`](test_guidelines.md) and [`Tests/README.md`](../Tests/README.md).
