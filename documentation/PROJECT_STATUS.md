...
- Product catalog with filters (price, sort — no shoe categories), pagination, and **server-side search** (`POST /api/products/search`, **10 results** per search request, `pg_trgm` on Supabase); optional `?q=` on `/products` applies text search — **no search bar on Home or Products**; **one product row per shoe** (all standard UK/US/EU sizes on detail/checkout)
...
- Contact form (client-side WhatsApp — prefilled message; Mohali store address); legal pages including no-refund / manufacturing-defect replacement policy (`/returns-policy`, `/replacement-policy`); **Terms of Service** governed by **Punjab, India** (`/terms-of-service`); **shipping policy aligned with checkout** ($25 / free over $200); cookie consent; GA4 and activity tracking (consent-gated)
...
- Coupons: create with **applies_to** scope (`all` or specific product IDs), edit modal, activate/deactivate, delete; **paginated list (10/page)**
