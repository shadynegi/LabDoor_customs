...
| Area | How it works |
|------|----------------|
| **Checkout** | Cart in localStorage; `policy_accepted` required; **Place Order** → WhatsApp redirect (`Order ID` = `orders.id` UUID); track-order links pre-fill `?orderId=` on `/orders`. |
