...
| Area | How it works |
|------|----------------|
| **Checkout** | Cart in localStorage; `policy_accepted` required; **Place Order** → WhatsApp redirect (`Order ID` = `orders.id` UUID); confirmation email links pre-fill `?orderId=` on `/orders`. |
| **Admin** | Bulk updates max **500** IDs; **Mark paid** with external payment reference; paid orders cannot cancel without refund; product cards on mobile. |
...
| **Mobile** | Sticky CTAs with keyboard lift on checkout; cookie banner top on purchase routes; cart stacked CTA + policy spacer; whole-number shoe sizes; Playwright **responsive-pages-ui** matrix; OOS hides product sticky bar; admin product cards on phones. |
