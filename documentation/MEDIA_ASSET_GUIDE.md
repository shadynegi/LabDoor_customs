...
| Area | How it works |
|------|----------------|
| **Checkout** | Cart in localStorage; `policy_accepted` required; **Place Order** → WhatsApp redirect (`Order ID` = `orders.id` UUID); order tracking links use `GET /api/orders/access-exchange/:code` (no token in email URL). |
