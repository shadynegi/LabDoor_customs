...
| **Dark mode** | Independent storefront and admin Sun/Moon toggles (44×44); storefront sets `data-theme` on `<html>` (`ldc_storefront_theme`), admin sets `data-admin-theme` on `.admin-root` (`ldc_admin_theme`); CSS custom-property tokens in `tokens.css`, synchronous init from localStorage/`prefers-color-scheme` (no theme flash). All text uses theme-aware tokens (secondary text tuned for ~7:1/~8:1 contrast); `color-scheme` pinned per scope so native controls (date pickers, dropdowns, scrollbars) follow the app toggle, not the OS. Modals/popovers adapt via the shared `LiquidModal` panel + tokenized dialogs; the Photo/Spin viewer toggle uses a dark translucent fill so both buttons stay visible over any product background. |
